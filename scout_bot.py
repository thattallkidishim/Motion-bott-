...
def fetch_gecko_networks():
    try:
        r = requests.get("https://api.geckoterminal.com/api/v2/networks", timeout=10,
...
def format_alert(token):
    ...
    if token.get("telegram"):
        socials.append(f"✈️ [Telegram]({token['telegram']})")
    if token.get("website"):
        socials.append(f"🌐 [Website]({token['website']})")
    socials_str = "  |  ".join(socials) if socials else "❌ No socials found"
    return (
        ...
        f"💬 _Pitch now: Community Manager / Raider / Project Manager_"
    )
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔔 Auto-Alerts", callback_data="subscribe")],
    ]
    await update.message.reply_text(
        "👋 *Crypto Launch Scout Bot*\n\n"
        "I scan DexScreener + GeckoTerminal across ALL chains for brand new launches so you can pitch early.\n\n"
        "🎯 *Filters:*\n"
        "• Under 2 hours old\n"
        "• $5K–$30K liquidity\n"
        "• $3K+ volume\n"
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.effective_message.reply_text("🔍 Scanning all chains... 30–60 secs.")
    leads = await run_full_scan()
    if not leads:
        await msg.edit_text("😔 No leads right now. Try again in a few minutes!")
        return
    await msg.edit_text(f"✅ Found *{len(leads)} lead(s)!*", parse_mode="Markdown")
    for token in leads[:20]:
        try:
            await update.effective_message.reply_text(
                format_alert(token), parse_mode="Markdown", disable_web_page_preview=True)
        except Exception as e:
...
async def cmd_subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    subscribed_chats.add(update.effective_chat.id)
    await update.effective_message.reply_text(
        f"🔔 Subscribed! Auto-alerts every *{SCAN_INTERVAL_MINUTES} minutes*.",
        parse_mode="Markdown")
